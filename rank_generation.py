...
if args.compress_rate:
    ...
    default_cprate={
      #  'vgg_16_bn': [0.7]*7+[0.1]*6,
        ...
        'resnet_56':[0.1]+[0.60]*35+[0.0]*2+[0.6]*17,
      #  'resnet_110':[0.1]+[0.40]*36+[0.40]*36+[0.4]*36
        'resnet_110':[0.1]+[0.40]*36+[0.45]*36+[0.5]+[0.6]*35
    }
...
if args.resume:
    ...
    # print("args.adjust_ckpt:",args.adjust_ckpt)
    if args.adjust_ckpt:
...
def get_feature_hook(self, input, output):
    ...
    global entropy
    global total
    a = output.shape[0] # batchsize
    b = output.shape[1] # channel 
    # 求每一个样本的特征图的每一个channel的秩
    c = torch.tensor([torch.matrix_rank(output[i,j,:,:]).item() for i in range(a) for j in range(b)])

    c = c.view(a, -1).float() # c.size() = torch.Size([a,b])
    c = c.sum(0) # c.size() = torch.Size([b])
    feature_result = feature_result * total + c # 更新每个通道秩的总数
    total = total + a # 总样本数累加
    feature_result = feature_result / total # 更新每个通道秩的均值
...
def test():
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            outputs = net(inputs) # 紧接着运行 get_feature_hook 179行
